...
def create(request):
    if request.method == 'POST':
        if request.POST['title'] and request.POST['body'] and request.POST['url'] and request.FILES['icon'] and request.FILES['image'] :
            ...
            return render (request,'products/create.html', {'error':'fill all the options'})

    else:
        return render (request,'products/create.html' )

def detail(request, product_id ):
    detailproduct = get_object_or_404( Products , pk=product_id )
    return render (request,'products/detail.html',{'products':detailproduct})
